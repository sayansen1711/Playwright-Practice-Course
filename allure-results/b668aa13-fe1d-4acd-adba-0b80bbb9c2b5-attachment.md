...
- Name: POM\POM_Test.spec.ts >> Demoblaze test >> TC-03: Successful Login
- Location: tests\POM\POM_Test.spec.ts:43:5
...
Expected: "testUser_1788278579184"
Received: "Welcome testUser_1788278579184"
```
    ...
    - list [ref=e6]:
      ...
      - listitem [ref=e16]:
        - link "Log out" [ref=e17] [cursor=pointer]:
          - /url: "#"
      - listitem [ref=e18]:
        - link "Welcome testUser_1788278579184" [ref=e19] [cursor=pointer]:
          - /url: "#"
      - listitem
    - generic [ref=e21]:
      - list [ref=e22]:
...
```ts
  ...
  34 |     //     // signedUpUser = {
  35 |     //     //     username: signedUpUser!.username,
  36 |     //     //     password: signedUpUser!.password
  37 |     //     // };
  38 |     //     const alertMessage = await signUpPage.signUp(signedUpUser!.username, signedUpUser!.password);
  39 |     //     expect(alertMessage).toContain('This user already exist');
  40 |     // });
  41 | 
  42 | 
  43 |     test('TC-03: Successful Login', async ({ page }) => {
  44 |         expect(signedUpUser).toBeDefined(); //Ensures that signedUpUser value is not undefined
  45 |         const loginPage = new LoginPage(page);
  46 |         const homePage = new HomePage(page);
  47 |         const cartPage = new CartPage(page);
  48 | 
  49 |         await loginPage.navigateToLoginIn();
  50 |         await loginPage.login(signedUpUser!.username, signedUpUser!.password);
  51 | 
  52 |         await page.waitForTimeout(3000);
  53 |         
> 54 |         expect(await homePage.getWelcomeUsername()).toBe(signedUpUser!.username);
     |                                                     ^ Error: expect(received).toBe(expected) // Object.is equality
  55 |     })
  56 | 
  57 | })
